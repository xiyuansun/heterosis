...
\begin{align*}
p \left ( {\sigma_g^2} \mid \cdots \right ) = \text{Gamma} \left (\text{shape} = N\frac{d \tau^2 + 3}{2} - 1, \  \text{rate} = \left (\frac{2N}{d} + \frac{1}{2} \sum_{n = 1}^N \e_{g, n}^2 \right )^{-1} \right)
\end{align*}

The $\sigma_g^2$'s will be sampled in parallel on the GPU.
