...
This gives us the (unsimplified) full conditional posteriors below, where $k(n)$ is the treatment group of library $n$. 
...
p(\sigma_c \mid \cdots) &= \prod_{n = 1}^N \prod_{g = 1}^G \text{N}(c_n \mid 0, \sigma_c^2) \cdot \text{U}(\sigma_c \mid 0, \sigma_{c0}) \\
p(\e_{g, n} \mid \cdots) &= \text{Poisson}(y_{g, n} \mid \exp(c_n + \e_{g, n} + \mu(n, \phi_g, \alpha_g, \delta_g))) \cdot \text{N}(\e_{g, n} \mid 0, \sigma_g^2) \\
p(\sigma_g^2 \mid \cdots) &= \prod_{n = 1}^N \text{N}(\e_{g, n} \mid 0, \sigma_g^2) \cdot \text{Inv-Gamma} \left ( \sigma_g^2 \mid \frac{d \cdot \tau^2}{2}, \frac{d}{2} \right ) \\ 
p(d \mid \cdots) &= \prod_{n = 1}^N \prod_{g = 1}^G \text{Inv-Gamma} \left ( \sigma_g^2 \mid \frac{d \cdot \tau^2}{2}, \frac{d}{2} \right ) \cdot \text{U}(d \mid 0, d_0) \\ 
p(\tau^2 \mid \cdots) &= \prod_{n = 1}^N \prod_{g = 1}^G \text{Inv-Gamma} \left ( \sigma_g^2 \mid \frac{d \cdot \tau^2}{2}, \frac{d}{2} \right ) \cdot \text{Gamma}(\text{shape} = a_\tau, \text{rate} = b_\tau) \\
