...
\title{GPU-parallel Gibbs sampling of a hierarchical model for hybrid vigor in gene expression}
\author{Will Landau}
\date{\today}
...
\pause \item $\eta_{g}$'s 
\end{itemize}
\pause \item Use parallel reductions to calculate sufficient statistics for:
\begin{itemize}
\pause \item $c_n$'s
\pause \item $\tau$, $d$
