...
\begin{align*}
&y_{g,n} \stackrel{\text{ind}}{\sim} \text{Poisson}(\exp(c_n + \e_{g, n} + \mu(g, n))) \\
...
& \qquad \qquad \qquad \tau^2 \stackrel{\text{}}{\sim} \text{Gamma}(\text{shape} = a_\tau, \text{rate} = b_\tau) \\
...
%& \qquad \delta_g \stackrel{\text{ind}}{\sim} \pi_\delta^{1-I(\delta_g)}[(1- \pi_\delta)\text{N}(\delta_g \mid \theta_\delta, \sigma_\delta^2)]^{I(\delta_g)} \\& \qquad \qquad \theta_\delta \stackrel{\text{}}{\sim} \text{N}(0, \gamma_{\delta}^2) \\
& \qquad \qquad \theta_\delta \stackrel{\text{}}{\sim} \text{N}(0, \gamma_{\delta}^2) \\
...
\item  $\mu(g, n)$ is the function given by:
\begin{align*}
\mu(g, n) = \begin{cases}
...
\item $\lambda_{g, n} = \exp(c_n + \e_{g, n} + \mu(g, n))$ )
...
\begin{align*}
p(c_n \mid \cdots) &\propto \exp \left (c_n G\ov{y}_{.n}  -\exp(c_n) \sum_{g = 1}^G \exp( \e_{g, n} + \mu(g, n)) - \frac{c_n^2}{2 \sigma_c^2} \right ) \\
p (\e_{g, n} \mid \cdots) &\propto \exp \left (y_{g, n} \e_{g, n} - \exp(c_n + \e_{g, n} + \mu(g, n))  - \frac{\e_{g, n}^2}{2 \eta_g^2} \right) \\
p \left ( \left . \frac{1}{\sigma_c^2}  \ \right \rvert  \ \cdots \right) &= \text{Gamma} \left (\text{shape} = \frac{N - 1}{2}, \ \text{rate} =\frac{1}{2} {\sum_{n = 1}^N c_n^2} \right )  I \left (\frac{1}{\sigma_c^2} > \frac{1}{\sigma_{c0}^2} \right ) \\
p \left ( \left . \frac{1}{\eta_g^2} \ \right \rvert \  \cdots \right ) &= \text{Gamma} \left (   \text{shape} = \frac{N + d}{2}, \  \text{rate} = \frac{1}{2} \left ( d \cdot \tau^2 + \sum_{n  =1}^N \e_{g, n}^2 \right ) \right ) \\
...
p(\tau^2 \mid \cdots) &= \text{Gamma} \left (  \text{shape} =  a_\tau + \frac{Gd}{2} , \ \text{rate} =  b_\tau + \frac{d}{2} \sum_{g = 1}^G \frac{1}{\eta_g^2} \right ) \\
p(\phi_g \mid \cdots) & \propto \exp \left (\sum_{n = 1}^N \left [y_{g, n} \mu(g, n)  - \exp (c_n + \e_{g, n} + \mu(g, n)) \right ] - \frac{(\phi_g - \theta_\phi)^2}{2 \sigma_\phi^2} \right ) \\
p(\alpha_g \mid \cdots) & \propto \exp \left (\sum_{n = 1}^N \left [y_{g, n} \mu(g, n)  - \exp (c_n + \e_{g, n} + \mu(g, n)) \right ] - \frac{(\alpha_g - \theta_\alpha)^2}{2 \sigma_\alpha^2} \right ) \\
p(\delta_g \mid \cdots) & \propto \exp \left (\sum_{n = 1}^N \left [y_{g, n} \mu(g, n)  - \exp (c_n + \e_{g, n} + \mu(g, n)) \right ] - \frac{(\delta_g - \theta_\delta)^2}{2 \sigma_\delta^2} \right ) \\
%p(\alpha_g \mid \cdots) &\propto \exp \left ( \sum_{k(n) \ne 2} \left [ y_{g, n} \cdot \mu(g, n) - \exp(c_n + \e_{g, n} + \mu(g, n)) \right ] \right . \\
...
%\end{align*}
%\begin{align*}
%p(\phi_g, \alpha_g, \delta_g \mid \cdots)  &\propto  \exp \left (\sum_{n = 1}^N \left [y_{g, n} \mu(g, n)  - \exp (c_n + \e_{g, n} + \mu(g, n)) \right ] - \frac{(\phi_g - \theta_\phi)^2}{2 \sigma_\phi^2} \right . \\
...
\end{align*}
\begin{align*}
p \left ( \left . \frac{1}{\sigma_\phi^2}  \ \right \rvert \ \cdots \right ) &= \text{Gamma} \left ( \text{shape} = \frac{G - 1}{2}, \ \text{rate} =  \frac{1}{2} \sum_{g = 1}^G (\phi_g - \theta_\phi)^2  \right )   \text{I} \left (\frac{1}{\sigma_\phi^2} >\frac{1}{ \sigma_{\phi 0}^2} \right ) \\
p \left ( \left . \frac{1}{\sigma_\alpha^2}  \ \right \rvert \ \cdots \right ) &= \text{Gamma} \left ( \text{shape} = \frac{G_\alpha - 1}{2}, \ \text{rate} = \frac{1}{2} \sum_{\alpha_g \ne 0} (\alpha_g - \theta_\alpha)^2 \right )  \text{I} \left (\frac{1}{\sigma_\alpha^2} >\frac{1}{ \sigma_{\alpha 0}^2} \right )\\
p \left ( \left . \frac{1}{\sigma_\delta^2}  \ \right \rvert \ \cdots \right ) &= \text{Gamma} \left (  \text{shape} = \frac{G_\delta - 1}{2}, \ \text{rate} = \frac{1}{2} \sum_{\delta_g \ne 0} (\delta_g - \theta_\delta)^2 \right )  \text{I} \left (\frac{1}{\sigma_\delta^2} >\frac{1}{ \sigma_{\delta 0}^2} \right ) \\
...
\begin{center}
\includegraphics[scale=.5]{depend.png}
\end{center}
...
All that remains is to find $\log p(\vc{y} \mid \vc{\psi})$ for a given set of parameters, $\vc{\psi}$. Let $\lambda_{g, n} = \exp(c_n + \e_{g, n} + \mu(g, n))$, where 
...
\begin{align*}
p(c_n \mid \cdots) &\propto \left [ \prod_{g = 1}^G \text{Poisson}(y_{g, n} \mid \exp(c_n + \e_{g, n} + \mu(g, n))) \right ] \cdot  \text{N}(c_n \mid 0, \sigma_c^2) \\
p(\e_{g, n} \mid \cdots) &\propto \text{Poisson}(y_{g, n} \mid \exp(c_n + \e_{g, n} + \mu(g, n))) \cdot \text{N}(\e_{g, n} \mid 0, \eta_g^2) \\
...
p(\phi_g \mid \cdots) &\propto \left [ \prod_{n = 1}^N \text{Poisson}(y_{g, n} \mid \exp(c_n + \e_{g, n} + \mu(g, n))) \right ] \cdot \text{N}(\phi_g \mid \theta_\phi, \sigma_\phi^2) \\
p(\alpha_g \mid \cdots) &\propto \left [ \prod_{k(n) \ne 2} \text{Poisson}(y_{g, n} \mid \exp(c_n + \e_{g, n} + \mu(g, n))) \right ] \\
& \quad \times \pi_\alpha^{1-I(\alpha_g)}[(1- \pi_\alpha)\text{N}(\alpha_g \mid \theta_\alpha, \sigma_\alpha^2)]^{I(\alpha_g)}
\end{align*}

\begin{align*}
p(\delta_g \mid \cdots) &\propto \left [ \prod_{k(n) = 2} \text{Poisson}(y_{g, n} \mid \exp(c_n + \e_{g, n} + \mu(g, n))) \right ] \\
p(\phi_g, \alpha_g, \delta_g \mid \cdots) &\propto \left [ \prod_{n = 1}^N \text{Poisson}(y_{g, n} \mid \exp(c_n + \e_{g, n} + \mu(g, n))) \right ] \cdot \text{N}(\phi_g \mid \theta_\phi, \sigma_\phi^2) \\
...
&=\exp \left (\sum_{g = 1}^G  \left [ y_{g, n} (c_n + \e_{g, n} + \mu(g, n)) - \exp(c_n + \e_{g, n} + \mu(g, n)) \right ] - \frac{c_n^2}{2 \sigma_c^2} \right ) \\
&=\exp \left (c_n G\ov{y}_{.n} +  \sum_{g = 1}^G \left [ y_{g, n}( \e_{g, n} + \mu(g, n)) \right ]  - \sum_{g = 1}^G \exp(c_n + \e_{g, n} + \mu(g, n))- \frac{c_n^2}{2 \sigma_c^2} \right ) \\
&\propto\exp \left (c_n G\ov{y}_{.n}  -\exp(c_n) \sum_{g = 1}^G \exp( \e_{g, n} + \mu(g, n))- \frac{c_n^2}{2 \sigma_c^2} \right ) \\
\end{align*}
...
&= \exp \left (y_{g, n} (c_n + \e_{g, n} + \mu(g, n))- \exp(c_n + \e_{g, n} + \mu(g, n))  - \frac{\e_{g, n}^2}{2 \eta_g^2} \right) \\
&= \exp \left (y_{g, n} \e_{g, n} - \exp(c_n + \e_{g, n} + \mu(g, n))  - \frac{\e_{g, n}^2}{2 \eta_g^2} \right)
\end{align*}
...
& \propto \prod_{g = 1}^G \left [  \Gamma \left( d/2 \right )^{-1} \left ( \frac{d \cdot \tau^2}{2}\right ) ^ {d  /2 } \left ( { \eta_g^2} \right )^{ -(d/2 + 1)} \exp \left (- \frac{1}{ \eta_g^2}\frac{d \cdot \tau^2}{2} \right ) \right ] I(2 < d < d_0) \\
...
& = \exp \left (\sum_{n = 1}^N \left [y_{g, n} (c_n + \e_{g, n} + \mu(g, n))  - \exp (c_n + \e_{g, n} + \mu(g, n)) \right ] - \frac{(\phi_g - \theta_\phi)^2}{2 \sigma_\phi^2} \right ) \\
& \propto \exp \left (\sum_{n = 1}^N \left [y_{g, n} \mu(g, n)  - \exp (c_n + \e_{g, n} + \mu(g, n)) \right ] - \frac{(\phi_g - \theta_\phi)^2}{2 \sigma_\phi^2} \right ) 
\end{align*}

\subsection{$p (\alpha_g \mid \cdots) $: Metropolis}

\begin{align*}
p(\alpha_g \mid \cdots) &= \left [ \prod_{k(n) \ne 2} \text{Poisson}(y_{g, n} \mid \exp(c_n + \e_{g, n} + \mu(g, n))) \right ] \pi_\alpha^{1-I(\alpha_g)}[(1- \pi_\alpha)\text{N}(\theta_\alpha, \sigma_\alpha^2)]^{I(\alpha_g)}\\
...
&=  \exp \left ( \sum_{k(n) \ne 2} \left [ y_{g, n} (c_n + \e_{g, n} + \mu(g, n)) - \exp(c_n + \e_{g, n} + \mu(g, n)) \right ] \right . \\
&\left .  \qquad - I(\alpha_g) \left ( \frac{(\alpha_g - \theta_\alpha)^2}{2 \sigma_\alpha^2} + \log( 1 - \pi_\alpha) \right ) + (1 - I(\alpha_g)) \log \pi_\alpha  \right ) \\
...
\begin{align*}
p(\delta_g \mid \cdots) &= \left [ \prod_{k(n) \ne 2} \text{Poisson}(y_{g, n} \mid \exp(c_n + \e_{g, n} + \mu(g, n))) \right ] \pi_\delta^{1-I(\delta_g)}[(1- \pi_\delta)\text{N}(\theta_\delta, \sigma_\delta^2)]^{I(\delta_g)}\\
...
\section{Using normal approximations to the full conditional distributions to improve Metropolis proposals}

The full conditional densities of the $c_n$'s, $\e_{g, n}$'s, and $\phi_g$'s are in the form of a Poisson density times a normal density. When sampling from these full conditionals in the Metropolis-Hastings algorithm, we can uses normal approximations to these full conditionals as proposal distributions.  \q

Let $g(\theta) = \log (p(\theta \mid \cdots))$ be the log full conditional density of some parameter, $\theta$. Let $\wh{\theta}$ be some point estimate of $\theta$ (for example, the MLE). Then, a Taylor series approximation gives us

\begin{align*}
g(\theta) &\approx g(\wh{\theta}) + g'(\wh{\theta})(\theta - \wh{\theta}) + \frac{g''(\wh{\theta})}{2} (\theta - \wh{\theta})^2 \\
&= g(\wh{\theta}) + g'(\wh{\theta}) \theta - g'(\wh{\theta}) \wh{\theta} + \frac{g''(\wh{\theta})}{2} \theta^2 - g''(\wh{\theta}) \wh{\theta} \theta + \frac{g''(\wh{\theta})}{2} \wh{\theta}^2 \\
&= \underbrace{\left [ g(\wh{\theta}) - g'(\wh{\theta}) \wh{\theta} + \frac{g''(\wh{\theta})}{2} \wh{\theta}^2 \right ]}_{A} + \underbrace{\left [ g'(\wh{\theta}) - g''(\wh{\theta}) \wh{\theta} \right ]}_{B} + \frac{g''(\wh{\theta})}{2} \theta^2 \\
&= A \left ( \theta + \frac{B}{2A} \right )^2 + C - \frac{B^2}{4A} 
\end{align*}

That means

\begin{align*}
\exp(g(\theta)) &\approx \exp \left [ A \left ( \theta + \frac{B}{2A} \right )^2 + C - \frac{B^2}{4A} \right ] \\
&\propto \exp \left [ A \left ( \theta + \frac{B}{2A} \right )^2 \right ] \\ 
&= \exp \left [ \frac{(\theta + \frac{B}{2A})^2}{2(2A) \nv} \right ]  \\
&\propto N \left ( -\frac{B}{2A}, \ \frac{1}{2A} \right ) \\
\end{align*}

Now,

\begin{align*}
-\frac{B}{2A} &= \frac{-g'(\wh{\theta}) + g''(\wh{\theta}) \wh{\theta}}{2g(\wh{\theta}) - 2g'(\wh{\theta}) \wh{\theta} + g''(\wh{\theta}) \wh{\theta}^2} \\
\frac{1}{2A} &= \frac{1}{2g(\wh{\theta}) - 2g'(\wh{\theta}) \wh{\theta} + g''(\wh{\theta}) \wh{\theta}^2}
\end{align*}

so that

\begin{align*}
\exp(g(\theta)) \approx N \left (\frac{-g'(\wh{\theta}) + g''(\wh{\theta}) \wh{\theta}}{2g(\wh{\theta}) - 2g'(\wh{\theta}) \wh{\theta} + g''(\wh{\theta}) \wh{\theta}^2}, \  \frac{1}{2g(\wh{\theta}) - 2g'(\wh{\theta}) \wh{\theta} + g''(\wh{\theta}) \wh{\theta}^2} \right ) 
\end{align*}

Let $q(\theta)$ be the above normal density and let $\theta^{(i)}$ be the current value of $\theta$ at iteration $i$ of the MCMC. To get $\theta^{(i + 1)}$, we first sample a proposal $\theta^*$ from $q$. Then, we compute the probability,

\begin{align*}
p &= \min \left ( 1,  \ \frac{p(\theta^* \mid \cdots) q(\theta^{(i)})}{p(\theta^{(i)} \mid \cdots ) q(\theta^*)} \right )
\end{align*}


We set $\theta^{(i + 1)} = \theta^*$ with probability $p$ and $\theta^{(i + 1)} = \theta^{(i)}$ with probability $1-p$. \q


In this approach, all that remains is to find $\wh{\theta}$. 


\subsection{Calculating $\wh{c}_n$}

Let $g(c_n)$ be the kernel of the log full conditional density of $c_n$. Then,

\begin{align*}
g(c_n) &= c_n G \ov{y}_{.n} - \exp(c_n) \sum_{g = 1}^G \exp(\e_{g, n} + \mu(g, n)) - \frac{c_n^2}{2 \sigma_c^2}
\end{align*}

Differentiating,

\begin{align*}
g'(c_n) &= G \ov{y}_{.n} - \exp(c_n) \sum_{g = 1}^G \exp(\e_{g, n} + \mu(g, n)) - \frac{c_n}{\sigma_c^2}
\end{align*}

We let $\wh{c}_n$ be the root of this derivative.

\begin{align*}
0 &= G \ov{y}_{.n} - \exp(\wh{c}_n) \sum_{g = 1}^G \exp(\e_{g, n} + \mu(g, n)) - \frac{\wh{c}_n}{\sigma_c^2}
\end{align*}

Using a quadratic approximation to the exponential function,

\begin{align*}
0 &= G \ov{y}_{.n} - \left (1 + \wh{c}_n + \frac{\wh{c}_n^2}{2} \right ) \underbrace{ \sum_{g = 1}^G \exp(\e_{g, n} + \mu(g, n))}_S - \frac{\wh{c}_n}{\sigma_c^2} \\
&= \left (G \ov{y}_{.n} - S \right ) + \left (-S - \frac{1}{\sigma_c^2} \right ) \wh{c}_n + \left ( \frac{S}{2} \right ) \wh{c}_n^2
\end{align*}

Using the quadratic formula, we get

\begin{align*}
\wh{c}_n &= \frac{S + \frac{1}{\sigma_c^2} \pm \sqrt{ \left (S + \frac{1}{\sigma_c^2} \right )^2 - 2 S (G \ov{y}_{.n} -S)}}{S}
\end{align*}
...
\begin{align*}
g(\e_{g, n}) &= y_{g, n} \e_{g, n} - \exp (c_n + \e_{g, n} + \mu(g, n)) - \frac{\e_{g, n}^2}{2 \eta_g^2} \\ 
&= y_{g, n} \e_{g, n} - \exp (\e_{g,n}) \exp (c_n + \mu(g, n)) - \frac{\e_{g, n}^2}{2 \eta_g^2} \\ 
\end{align*}

Differentiating with respect to $\e_{g, n}$,

\begin{align*}
g(\e_{g, n}) &= y_{g, n} -  \exp (\e_{g,n}) \exp (c_n + \mu(g, n)) - \frac{\e_{g, n}}{\eta_g^2}
\end{align*}

We let $\wh{\e}_{g, n}$ be the root of this derivative.

\begin{align*}
0 &= y_{g, n} - \exp (\wh{\e}_{g,n}) \exp (c_n + \mu(g, n)) - \frac{\wh{\e}_{g, n}}{\eta_g^2}
\end{align*}

Taking the quadratic approximation to the exponential,

\begin{align*}
0 &= y_{g, n} - \left ( 1  + \wh{\e}_{g,n} + \frac{\wh{\e}_{g, n}}{2}\right ) \underbrace{\exp (c_n + \mu(g, n))}_{S} - \frac{\wh{\e}_{g, n}}{\eta_g^2} \\
&= (y_{g, n} - S) + \left (-S - \frac{1}{\eta_g^2} \right ) \wh{\e}_{g, n} + \left (\frac{
...
\begin{align*}
\wh{\e}_{g, n} &= \frac{\left (S + \frac{1}{\eta_g^2} \right )  \pm \sqrt{ \left (S + \frac{1}{\eta_g^2} \right )^2 - 2S(y_{g, n} - S)    }   }{S}
\end{align*}
...
\begin{align*}
g(\phi_g) &= \sum_{n = 1}^N \left [  y_{g, n} \mu(g, n) - \exp(c_n + \e_{g, n} - \mu(g, n)) \right ] - \frac{(\phi_g - \theta_\phi)^2}{2 {\sigma_\phi}^2} \\
&=  \sum_{\text{group}(n) = 1} \left [ y_{g, n}(\phi_g - \alpha_g) - \exp(c_n + \e_{g, n} - (\phi_g - \alpha_g)) \right ] \\
& \qquad+  \sum_{\text{group}(n) = 2} \left [ y_{g, n}(\phi_g + \delta_g) - \exp(c_n + \e_{g, n} - (\phi_g + \delta_g)) \right ] \\
&\qquad+  \sum_{\text{group}(n) = 3} \left [ y_{g, n}(\phi_g + \alpha_g) - \exp(c_n + \e_{g, n} - (\phi_g + \alpha_g)) \right ] \\
&\qquad- \frac{(\phi_g - \theta_\phi)^2}{2 \sigma_\phi^2} \\ \\
&=  \sum_{\text{group}(n) = 1} y_{g, n}(\phi_g - \alpha_g) -  \sum_{\text{group}(n) = 1} \exp(c_n + \e_{g, n} - (\phi_g - \alpha_g)) \\
& \qquad+  \sum_{\text{group}(n) = 2}  y_{g, n}(\phi_g + \delta_g) - \sum_{\text{group}(n) = 2}  \exp(c_n + \e_{g, n} - (\phi_g + \delta_g)) \\
&\qquad+  \sum_{\text{group}(n) = 3} y_{g, n}(\phi_g + \alpha_g) -  \sum_{\text{group}(n) = 3} \exp(c_n + \e_{g, n} - (\phi_g + \alpha_g))  \\
&\qquad- \frac{(\phi_g - \theta_\phi)^2}{2 \sigma_\phi^2} \\ \\
&=  \phi_g \sum_{\text{group}(n) = 1} y_{g, n} - \alpha_g \sum_{\text{group}(n) = 1} y_{g, n} -  \exp(-\phi_g) \sum_{\text{group}(n) = 1} \exp(c_n + \e_{g, n} + \alpha_g) \\
&\qquad+  \phi_g \sum_{\text{group}(n) = 2} y_{g, n} + \delta_g \sum_{\text{group}(n) = 2} y_{g, n}  - \exp(-\phi_g) \sum_{\text{group}(n) = 2}  \exp(c_n + \e_{g, n} -  \delta_g) \\
&\qquad+  \phi_g \sum_{\text{group}(n) = 3} y_{g, n} + \alpha_g \sum_{\text{group}(n) = 3} y_{g, n}  -  \exp(-\phi_g)\sum_{\text{group}(n) = 3} \exp(c_n + \e_{g, n} - \alpha_g)  \\
&\qquad- \frac{(\phi_g - \theta_\phi)^2}{2 \sigma_\phi^2} \\ \\
...
S &=\sum_{\text{group}(n) = 1} \exp(c_n + \e_{g, n} + \alpha_g) + \sum_{\text{group}(n) = 2} \exp(c_n + \e_{g, n} - \delta_g) + \sum_{\text{group}(n) = 3} \exp(c_n + \e_{g, n} - \alpha_g) 
\end{align*}
...
&=  \int_{\alpha_g \ne 0}  \text{Poisson}(y_{g, n} \mid \exp(c_n + \e_{g, n} + \mu(g, n)))  N(\alpha_g \mid \theta_\alpha', (\sigma_\alpha')^2) d \alpha_g \\
