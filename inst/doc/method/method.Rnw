...
\begin{enumerate}
\item Sample a proposal $\nu^*$ from a $N \left (\frac{h_2 \wh{\nu} - h_1}{h_2}, \ |h_2| \nv \right )$ distribution.
...
&= h_0 + h_1 \theta - h_1 \wh{\theta}  + \frac{h_2}{2} \theta^2  - h_2 \wh{\theta} \theta + \frac{h_2}{2} \wh{\theta}^2 \\
&= \left (h_0 - h_1 \wh{\theta} + \frac{h_2}{2} \wh{\theta}^2 \right ) + (h_1 - h_2 \wh{\theta}) \theta + \frac{h_2}{2} \theta^2 \\
&= \left (h_0 - h_1 \wh{\theta} + \frac{h_2}{2} \wh{\theta}^2 \right ) - \frac{(h_1 - h_2 \wh{\theta})^2}{2h_2} + \frac{h_2}{2} \left (\theta - \frac{h_2 \wh{\theta} - h_1}{h_2} \right )^2 \\
&=  -\frac{1}{2 |h_2| \nv} \left (\theta - \frac{h_2 \wh{\theta} - h_1}{h_2} \right )^2  + \text{ constant}
\end{align*}

Thus, a reasonable proposal distribution for $\theta$ in the Metropolis algorithm is 

\begin{align*}
N \left (\frac{h_2 \wh{\theta} - h_1}{h_2}, \ |h_2| \nv \right )
\end{align*}

Let

\begin{align*}
q(\theta) = -\frac{1}{2 |h_2| \nv} \left (\theta - \frac{h_2 \wh{\theta} - h_1}{h_2} \right )^2 
\end{align*}

i.e., the log density of the normal distribution. Let $\theta^{(i)}$ be the current value of $\theta$ at iteration $i$ of the algorithm. To get $\theta^{(i + 1)}$,

\begin{enumerate}
\item Sample a proposal $\theta^*$ from a $N \left (\frac{h_2 \wh{\theta} - h_1}{h_2}, \ |h_2| \nv \right )$ distribution.
\item The acceptance probability is
