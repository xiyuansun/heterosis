...
&= \frac{1}{2} \log (\nu) + \nu \left ( \frac{1}{2} + K \right ) - \frac{1}{2} \log ( 4 \pi ) \\
&= \frac{1}{2} \log (\nu) + \nu \left ( \frac{1}{2} + \frac{ \log (\tau^2 )}{2} -   \frac{1}{G} \sum_{g = 1}^G \left [ \log \gamma_g + \frac{\tau^2}{2} \frac{1}{\gamma_g^2} \right ] \right ) - \frac{1}{2} \log ( 4 \pi ) \\
\end{align*}
...
Let $q(\nu)$ be the log kernel of this gamma approximation to $p(\nu \mid \cdots)$. Then, we have a Metropolis-Hastings algorithm for sampling $\nu$.
...
Unfortunately, the tails of this normal approximation are too thin for the proposal to be efficient. However, we can instead sample $\sqrt{|h_2|}(\theta - \wh{\theta})$ from a $t_d$ distribution, where the degrees of freedom, $d$, is some large positive number. Let $q(\theta)$ be the log density of $\theta$ derived from this $t_d$ approximation. Then, for a proportionality constant, $k$, the ratio of the approximate density to the true full conditional density is

\begin{align*}
\log \frac{\exp(q(\theta))}{p(\theta \mid \cdots)} &= \log q(\theta) - \log p(\theta \mid \cdots)  \\
&= \log \left [ k \left ( 1 + \frac{\sqrt{|h_2|}(\theta - \wh{\theta})^2}{d} \right )^{-\frac{d + 1}{2}} \right ]  - \left [  A \theta - B(\theta - C)^2 - D e^{\theta} - Ee^{-\theta} \right ] \\
&=  \log(k) - \frac{d + 1}{2} \log \left ( 1 + \frac{\sqrt{|h_2|}(\theta - \wh{\theta})^2}{d} \right ) -  A \theta + B(\theta - C)^2 + D e^{\theta} + Ee^{-\theta} \\
\end{align*} 
...
Thus, the proposal has thicker tails than the full conditional. Moreover, for a large $d$, the $t_d$ density well approximates the full conditional, as seen in an example plot below.
...
Hence, the $t_d$ proposal should be efficient.
