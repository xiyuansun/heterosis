...
i.e., the log density of the normal distribution. We see that the normal approximation has thick enough tails to be efficient:
...
If $E > 0$, $\log \frac{\exp(q(\theta))}{p(\theta \mid \cdots)} \rt \infty$ as  $|\theta| \rt \infty$. If $E = 0$ and $D > 0$, then $\log \frac{\exp(q(\theta))}{p(\theta \mid \cdots)} \rt -\frac{1}{2 |h_2| \nv} \left (\theta - \wh{\theta} \right )^2  -  A \theta + B(\theta - C)^2 \rt \infty$ as $\theta \rt -\infty$. There are no other cases. Thus, the proposal has thicker tails than the full conditional. \q

For the full sampler, let $\theta^{(i)}$ be the current value of $\theta$ at iteration $i$ of the algorithm. To get $\theta^{(i + 1)}$,

\begin{enumerate}
\item Sample a proposal $\theta^*$ from a $N \left (\wh{\theta}, \ |h_2| \nv \right )$ distribution.
\item The acceptance probability is
